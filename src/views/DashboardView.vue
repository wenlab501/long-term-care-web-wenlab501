    ...
    setup(props) {
      ...
      const salesData = computed(() => {
        if (!props.mergedTableData || props.mergedTableData.length === 0) {
          return [{ name: '無數據', value: 0, color: '#6c757d' }];
        }
      ...
      const marketData = computed(() => {
        if (!props.mergedTableData || props.mergedTableData.length === 0) {
          return [{ name: '無數據', value: 100, color: '#6c757d' }];
        }
      ...
      const createPieChart = () => {
        ...
        arcs
          ...
          .text(
            (d) =>
              `${d.data.name}: ${d.data.value} (${((d.data.value / d3.sum(marketData.value, (d) => d.value)) * 100).toFixed(1)}%)`
          );
        ...
        arcs
          ...
          .text((d) => {
            const percent = (
              (d.data.value / d3.sum(marketData.value, (d) => d.value)) *
              100
            ).toFixed(1);
        ...
        (dragging) => {
          nextTick(() => {
            if (barChart.value) {
              ...
              console.log(
                `DashboardView: 柱狀圖指針事件設為 ${barChart.value.style.pointerEvents}`
              );
            }
            if (pieChart.value) {
              pieChart.value.style.pointerEvents = dragging ? 'none' : 'auto';
              console.log(
                `DashboardView: 圓餅圖指針事件設為 ${pieChart.value.style.pointerEvents}`
              );
            }
          });
        },
