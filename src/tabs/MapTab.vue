          ...
          .forEach((layer) => {
            ...
            try {
              ...
              if (newLayer) {
                ...
                console.log(`🗺️ 普通圖層 "${layer.layerTitle + (layer.layerSubtitles?.[0] || '')}" 已添加到地圖`);
              }
            } catch (error) {
              console.error(`添加普通圖層 "${layer.layerTitle + (layer.layerSubtitles?.[0] || '')}" 時發生錯誤:`, error);
            }
          });
          ...
          .forEach((layer) => {
            ...
            try {
              ...
              if (newLayer) {
                ...
                console.log(`🗺️ 分析圖層 "${layer.layerTitle + (layer.layerSubtitles?.[0] || '')}" 已添加到地圖（最上層）`);
              }
            } catch (error) {
              console.error(`添加分析圖層 "${layer.layerTitle + (layer.layerSubtitles?.[0] || '')}" 時發生錯誤:`, error);
            }
          });
