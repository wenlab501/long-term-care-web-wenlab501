...
export async function loadMembershipGeoJson(layer) {
  try {
    const layerId = layer.layerId;
    const activeIndex = layer.activeFieldIndex ?? 0;
    const fieldName = layer.layerFields?.[activeIndex]?.fieldName; // 應該是 'Membership' (群體感)
...
export async function loadInfluenceGeoJson(layer) {
  try {
    const layerId = layer.layerId;
    const activeIndex = layer.activeFieldIndex ?? 0;
    const fieldName = layer.layerFields?.[activeIndex]?.fieldName; // 應該是 'Influence' (影響力)
...
export async function loadCPRGeoJson(layer) {
  try {
    const layerId = layer.layerId;
    const activeIndex = layer.activeFieldIndex ?? 0;
    const fieldName = layer.layerFields?.[activeIndex]?.fieldName; // 應該是 'tot_CPR' (協助執行CPR意願)
...
export async function loadAEDGeoJson(layer) {
  try {
    const layerId = layer.layerId;
    const activeIndex = layer.activeFieldIndex ?? 0;
    const fieldName = layer.layerFields?.[activeIndex]?.fieldName; // 應該是 'tot_AED' (協助執行AED意願)
